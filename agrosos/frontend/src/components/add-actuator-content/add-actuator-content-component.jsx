...
function AddActuatorContentComponent() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await fetch('http://localhost:3000/api/actuators', { // Ajusta la URL si es diferente
                method: 'POST',
                headers: {
            ...
            setSuccessMessage(`Actuador "${actuatorName}" con código "${actuatorCode}" enlazado con éxito.`); // Mensaje de éxito
    ...
    return (
        <div className="container-add-actuator"> {/* Asegúrate de tener esta clase en tu CSS */}
            <div className="form-add-actuator">
                ...
                <form className="actuator-form" onSubmit={handleSubmit}>
                    <div className="form-group-actuator-name">
                        <label htmlFor="actuator-name-input" className="label-actuator-name">Nombre del actuador</label>
                        <div className="actuator-input-container">
                            <input type="text" id="actuator-name-input" className="input-actuator-name" value={actuatorName} readOnly />
                            <img src={lock} alt="lock" className="lock-icon" />
                        </div>
                    </div>
                    <div className="form-group-actuator-code">
                        <label htmlFor="actuator-code-input" className="label-actuator-code">Código del actuador a enlazar</label>
                        <input type="number" id="actuator-code-input" className="input-actuator-code" placeholder="Ingrese el código" value={actuatorCode} onChange={(e) => setActuatorCode(e.target.value)} />
                    </div>
                    {error && <p className="error-message">{error}</p>}
                    {successMessage && <p className="actuator-success-message">{successMessage}</p>} {/* Mostrar mensaje de éxito */}
                    <div className="add-actuator-content-buttons">
                      <button type="submit" className="btn-enla">Enlazar</button>
                        <Link to="/actuators">
